...
def build_summary(results):
    ...
    priority = sorted(issues, key=lambda x: (x.get("priority_rank", 99), -x["max_points"]))
    ...
    if not issues:
        ...
        headline = "The website has a good security baseline, with a few improvements to make."
        message = (
            ...
            "explains what the problem means and gives you a practical configuration example."
        )
    elif score >= 40:
        headline = "The website has some protection, but important gaps remain."
        message = (
            f"We verified {verified} of {len(results)} protections. {len(issues)} "
            "item(s) need attention. You do not need to understand HTTP headers first "
            "—the fixes below explain what to change and why."
        )
    else:
        headline = "Several important website protections are missing or need correction."
        message = (
            f"Only {verified} of {len(results)} protections were verified. This does not "
            "mean the website is hacked. It means this tool found missing or weak "
            "browser-level protections. Work through the fixes from top to bottom and "
            "scan the website again after making changes."
        )
...
def _is_public_ip(value):
    ...
    # unspecified and other non-public ranges.  This also blocks cloud
...
def _resolve_public_host(hostname):
    ...
    Rejecting a hostname if ANY returned address is non-public prevents a
    mixed public/private DNS answer from becoming an SSRF bypass.
    """
    hostname = hostname.rstrip(".").lower()

    if not hostname or hostname == "localhost" or hostname.endswith(".localhost"):
        raise ValueError("Localhost and local network targets are not allowed.")
    ...
    try:
        ...
        # A ValueError here may simply mean this is a DNS hostname. Preserve
        # explicit security failures for IP literals.
...
def _validate_target_url(raw_url):
    ...
    try:
        ...
        raise ValueError("The website address is not valid.") from exc

    if parsed.scheme.lower() not in {"http", "https"}:
        raise ValueError("Only HTTP and HTTPS websites are allowed.")
    ...
    try:
        ...
        raise ValueError("The target uses an invalid port.") from exc
    ...
    # Normalize the URL while preserving path/query. Fragments are never sent
    # to the server and are removed from the scanner target.
    return urlunsplit(
        (
            parsed.scheme.lower(),
            parsed.netloc,
...
def fetch_headers_safely(start_url):
    ...
    The connection is pinned to an IP resolved immediately before each
    request. HTTPS keeps the original hostname for SNI/certificate checking,
    while the socket connects to the validated public IP.
    """
    ...
    while True:
        ...
        try:
            ...
            # We intentionally never consume the body: the analyzer only needs
            # status + headers, which keeps serverless bandwidth and memory use low.
            if response.status in {301, 302, 303, 307, 308}:
                ...
                current_url = _validate_target_url(next_url)
                redirects += 1
                continue
            ...
            response.release_conn()
            pool.close()
            return current_url, status_code, safe_headers

        except urllib3.exceptions.SSLError as exc:
            pool.close()
            raise ValueError("The website's HTTPS certificate could not be verified.") from exc
            ...
            pool.close()
            raise ConnectionError("Could not connect to the website.") from exc

    if last_response is not None:
        try:
            safe_headers = _safe_response_headers(last_response.headers)
            status_code = int(last_response.status)
            last_response.release_conn()
        finally:
            pool.close()
        return current_url, status_code, safe_headers

    raise ConnectionError("Could not retrieve the website response headers.")
...
def add_security_headers(response):
    ...
    # Vercel production deployments are HTTPS, while local HTTP development is not.
...
def index():
    ...
    if request.method == "POST":
        ...
        try:
            final_url, status_code, headers = fetch_headers_safely(raw_url)
            url = final_url
            security_results, security_score = analyze_headers(headers)
            grade, grade_label = get_grade(security_score)
            security_summary = build_summary(security_results)
            ...
            error = "The scan could not be completed. Please check the website address and try again."
